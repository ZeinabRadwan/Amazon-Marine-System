<?php

namespace Database\Seeders;

use App\Models\ShipmentStatus;
use Illuminate\Database\Seeder;

class ShipmentOperationalStatusesSeeder extends Seeder
{
    public function run(): void
    {
        $statuses = [
            [
                'name_ar' => 'جاري تخصيص وسحب الحاويات',
                'name_en' => 'Allocating and Pulling Containers',
                'type' => 'operational',
                'color' => '#3B82F6',
                'sort_order' => 1,
            ],
            [
                'name_ar' => 'جاري التحميل',
                'name_en' => 'Loading in Progress',
                'type' => 'operational',
                'color' => '#F59E0B',
                'sort_order' => 2,
            ],
            [
                'name_ar' => 'جاري التخليص الجمركي',
                'name_en' => 'Customs Clearance in Progress',
                'type' => 'operational',
                'color' => '#8B5CF6',
                'sort_order' => 3,
            ],
            [
                'name_ar' => 'بانتظار فواتير الشحن',
                'name_en' => 'Awaiting Shipment Invoices',
                'type' => 'operational',
                'color' => '#6366F1',
                'sort_order' => 4,
            ],
            [
                'name_ar' => 'جاري الفوترة',
                'name_en' => 'Invoicing in Progress',
                'type' => 'operational',
                'color' => '#EC4899',
                'sort_order' => 5,
            ],
            [
                'name_ar' => 'بانتظار دفع العميل',
                'name_en' => 'Awaiting Client Payment',
                'type' => 'operational',
                'color' => '#EF4444',
                'sort_order' => 6,
            ],
            [
                'name_ar' => 'جاري الإفراج عن الشحنة',
                'name_en' => 'Shipment Release in Progress',
                'type' => 'operational',
                'color' => '#10B981',
                'sort_order' => 7,
            ],
            [
                'name_ar' => 'مكتملة',
                'name_en' => 'Shipment Completed',
                'type' => 'operational',
                'color' => '#059669',
                'sort_order' => 8,
            ],
        ];

        foreach ($statuses as $status) {
            ShipmentStatus::updateOrCreate(
                ['name_en' => $status['name_en'], 'type' => 'operational'],
                $status
            );
        }
    }
}
